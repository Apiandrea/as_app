...
// Recupera anche il valore di is_admin per l'utente loggato
...
// Elimina post se l'utente è admin
if (isset($_POST['delete_post_id']) && $_SESSION['is_admin']) {
...
$result = $conn->query("SELECT p.id, p.title, p.content, p.color, u.username, p.created_at, p.updated_at FROM posts p JOIN users u ON p.user_id = u.id ORDER BY p.id DESC LIMIT $postsPerPage OFFSET $offset");
...
<div class="container mt-5">
    ...
    <div class="row">
        <div class="col-md-6 mb-4">
            <div class="card">
                <div class="card-header">
                    <h4 class="text-center">Crea Post</h4>
                </div>
                <div class="card-body">
                    <?php if (isset($error)): ?>
                    ...
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="content" maxlength="200" rows="5" required></textarea>
                        </div>
                        <div class="mb-3">
                            <label for="color" class="form-label">Colore</label>
                            <input type="color" name="color" value="#00b4d8" class="form-control" required>
                        </div>
        ...
        <div class="col-md-6 mb-4">
            <div class="card">
                <div class="card-header">
                    <h4 class="text-center">Post Recenti</h4>
                </div>
                <div class="card-body">
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <div class="post mb-3 p-3" style="border-left: 10px solid <?php echo htmlspecialchars($row['color']); ?>;">
                            <h5><?php echo htmlspecialchars($row['title']); ?></h5>
                            <small class="text-muted">di <?php echo htmlspecialchars($row['username']); ?> creato il <?php echo htmlspecialchars($row['created_at']); ?></small>
                            ...
                            <?php if ($_SESSION['is_admin'] || $row['username'] === $_SESSION['username']): ?>
                                <form method="POST" onsubmit="return confirm('Sei sicuro di voler eliminare questo post?');">
                                    ...
                                    <a href="edit_post.php?id=<?php echo $row['id']; ?>" class="btn btn-secondary btn-sm mt-2 ms-2">
